    ...
    describe(name, () => {
      ...
      describe('basic list sorting', () => {
        ...
        it('sorts strings descending with ALPHA DESC', async () => {
          await storage.rpush('mylist', ['c', 'a', 'b']);
          const result = await storage.sort('mylist', { alpha: true, sortOrder: 'DESC' }) as string[];
          expect(result).toEqual(['c', 'b', 'a']);
        });

        it('throws error for non-numeric values without ALPHA', async () => {
          await storage.rpush('mylist', ['a', 'b', 'c']);
          await expect(storage.sort('mylist')).rejects.toThrow("One or more scores can't be converted into double");
        });
      ...
      describe('BY pattern', () => {
        ...
        it('sorts by external weights descending', async () => {
          ...
          await storage.set('weight_b', '1');
          await storage.set('weight_c', '2');
          const result = await storage.sort('mylist', { byPattern: 'weight_*', sortOrder: 'DESC' }) as string[];
        ...
        it('sorts by external weights with ALPHA', async () => {
          ...
          const result = await storage.sort('mylist', { byPattern: 'weight_*', alpha: true }) as string[];
          expect(result).toEqual(['b', 'c', 'a']);
        });
      ...
      describe('GET patterns', () => {
        ...
        it('sorts DESC and retrieves values with GET', async () => {
          ...
          await storage.set('obj_2', 'two');
          await storage.set('obj_3', 'three');
          const result = await storage.sort('mylist', { sortOrder: 'DESC', getPatterns: ['obj_*'] }) as string[];
        ...
        it('supports multiple GET patterns', async () => {
          ...
          const result = await storage.sort('mylist', { getPatterns: ['obj_*', 'extra_*'] }) as string[];
      ...
      describe('LIMIT', () => {
        it('applies LIMIT offset and count', async () => {
          ...
          const result = await storage.sort('mylist', { limit: { offset: 1, count: 2 } }) as string[];
        ...
        it('applies LIMIT with offset 0', async () => {
          await storage.rpush('mylist', ['3', '1', '2']);
          const result = await storage.sort('mylist', { limit: { offset: 0, count: 2 } }) as string[];
          expect(result).toEqual(['1', '2']);
        });

        it('applies LIMIT with DESC', async () => {
          await storage.rpush('mylist', ['5', '4', '3', '2', '1']);
          const result = await storage.sort('mylist', { sortOrder: 'DESC', limit: { offset: 0, count: 2 } }) as string[];
