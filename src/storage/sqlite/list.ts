...
export const listMethods = {
  _ensureListKvStoreEntry(key: string): void {
    ...
    if (!row) {
      this.db
        .prepare(
          "INSERT OR REPLACE INTO kv_store (key, value, type, expires_at) VALUES (?, '', 'list', NULL)"
        )
        .run(key);
    }
  },
  ...
  async lpush(key: string, elements: string[]): Promise<number> {
    ...
    const tx = this.db.transaction(() => {
      ...
      const minSeqRow = this.db
        .prepare('SELECT MIN(seq) as minSeq FROM list_store WHERE key = ?')
        .get(key) as { minSeq: number | null };
  ...
  async rpush(key: string, elements: string[]): Promise<number> {
    ...
    const tx = this.db.transaction(() => {
      ...
      const maxSeqRow = this.db
        .prepare('SELECT MAX(seq) as maxSeq FROM list_store WHERE key = ?')
        .get(key) as { maxSeq: number | null };
  ...
  async lpop(key: string, count?: number): Promise<string | string[] | null> {
    ...
    const tx = this.db.transaction(() => {
      if (count === undefined || count === 1) {
        const row = this.db
          .prepare('SELECT value FROM list_store WHERE key = ? ORDER BY seq ASC LIMIT 1')
        ...
        this.db
          .prepare(
            'DELETE FROM list_store WHERE key = ? AND seq = (SELECT seq FROM list_store WHERE key = ? ORDER BY seq ASC LIMIT 1)'
          )
          .run(key, key);
        this._cleanupListIfEmpty(key);
      ...
      const rows = this.db
        .prepare('SELECT value FROM list_store WHERE key = ? ORDER BY seq ASC LIMIT ?')
      ...
      for (const row of rows) {
        this.db
          .prepare(
            'DELETE FROM list_store WHERE key = ? AND value = ? AND seq = (SELECT seq FROM list_store WHERE key = ? AND value = ? ORDER BY seq ASC LIMIT 1)'
          )
          .run(key, row.value, key, row.value);
      }
      this._cleanupListIfEmpty(key);
  ...
  async rpop(key: string, count?: number): Promise<string | string[] | null> {
    ...
    const tx = this.db.transaction(() => {
      if (count === undefined || count === 1) {
        const row = this.db
          .prepare('SELECT value FROM list_store WHERE key = ? ORDER BY seq DESC LIMIT 1')
          .get(key) as { value: string } | undefined;
        if (!row) {
          this._cleanupListIfEmpty(key);
          return null;
        ...
        this.db
          .prepare(
            'DELETE FROM list_store WHERE key = ? AND seq = (SELECT seq FROM list_store WHERE key = ? ORDER BY seq DESC LIMIT 1)'
          )
          .run(key, key);
        this._cleanupListIfEmpty(key);
        return row.value;
      ...
      const rows = this.db
        .prepare('SELECT value FROM list_store WHERE key = ? ORDER BY seq DESC LIMIT ?')
        .all(key, count) as { value: string }[];
      if (rows.length === 0) {
        this._cleanupListIfEmpty(key);
        return null;
      ...
      for (const row of rows) {
        this.db
          .prepare(
            'DELETE FROM list_store WHERE key = ? AND value = ? AND seq = (SELECT seq FROM list_store WHERE key = ? AND value = ? ORDER BY seq DESC LIMIT 1)'
          )
          .run(key, row.value, key, row.value);
      }
      this._cleanupListIfEmpty(key);
  ...
  async lrange(key: string, start: number, stop: number): Promise<string[]> {
    ...
    const limit = e - s + 1;
    const rows = this.db
      .prepare('SELECT value FROM list_store WHERE key = ? ORDER BY seq ASC LIMIT ? OFFSET ?')
      .all(key, limit, s) as { value: string }[];
    return rows.map((r) => r.value);
  },
  ...
  async lindex(key: string, index: number): Promise<string | null> {
    ...
    const row = this.db
      .prepare('SELECT value FROM list_store WHERE key = ? ORDER BY seq ASC LIMIT 1 OFFSET ?')
      .get(key, idx) as { value: string } | undefined;
  ...
  async lset(key: string, index: number, element: string): Promise<void> {
    ...
    const tx = this.db.transaction(() => {
      ...
      const seqRow = this.db
        .prepare('SELECT seq FROM list_store WHERE key = ? ORDER BY seq ASC LIMIT 1 OFFSET ?')
        .get(key, idx) as { seq: number };
      this.db
        .prepare('UPDATE list_store SET value = ? WHERE key = ? AND seq = ?')
        .run(element, key, seqRow.seq);
    });
    tx();
  },
  ...
  async lrem(key: string, count: number, element: string): Promise<number> {
    ...
    const tx = this.db.transaction(() => {
      ...
      if (count > 0) {
        const rows = this.db
          .prepare(
            'SELECT seq FROM list_store WHERE key = ? AND value = ? ORDER BY seq ASC LIMIT ?'
          )
          .all(key, element, count) as { seq: number }[];
        for (const row of rows) {
        ...
        const rows = this.db
          .prepare(
            'SELECT seq FROM list_store WHERE key = ? AND value = ? ORDER BY seq DESC LIMIT ?'
          )
          .all(key, element, Math.abs(count)) as { seq: number }[];
        for (const row of rows) {
          this.db.prepare('DELETE FROM list_store WHERE key = ? AND seq = ?').run(key, row.seq);
        }
        removed = rows.length;
        ...
        const result = this.db
          .prepare('DELETE FROM list_store WHERE key = ? AND value = ?')
          .run(key, element);
  ...
  async ltrim(key: string, start: number, stop: number): Promise<void> {
    ...
    const tx = this.db.transaction(() => {
      ...
      const keepSeqs = this.db
        .prepare('SELECT seq FROM list_store WHERE key = ? ORDER BY seq ASC LIMIT ? OFFSET ?')
        .all(key, e - s + 1, s) as { seq: number }[];
      ...
      this.db
        .prepare(`DELETE FROM list_store WHERE key = ? AND seq NOT IN (${placeholders})`)
        .run(key, ...keepSeqs.map((r) => r.seq));
      this._cleanupListIfEmpty(key);
    });
    tx();
  },
  ...
  async rpoplpush(source: string, destination: string): Promise<string | null> {
    ...
    const tx = this.db.transaction(() => {
      const srcRow = this.db
        .prepare('SELECT seq, value FROM list_store WHERE key = ? ORDER BY seq DESC LIMIT 1')
        .get(source) as { seq: number; value: string } | undefined;
      ...
      const minSeqRow = this.db
        .prepare('SELECT MIN(seq) as minSeq FROM list_store WHERE key = ?')
        .get(destination) as { minSeq: number | null };
      const destSeq = minSeqRow.minSeq !== null ? minSeqRow.minSeq - 1 : 0;
      this.db
        .prepare('INSERT INTO list_store (key, seq, value) VALUES (?, ?, ?)')
        .run(destination, destSeq, srcRow.value);
  ...
  async linsert(
    ...
    const tx = this.db.transaction(() => {
      // Find pivot
      const pivotRow = this.db
        .prepare('SELECT seq FROM list_store WHERE key = ? AND value = ? ORDER BY seq ASC LIMIT 1')
        .get(key, pivot) as { seq: number } | undefined;
      if (!pivotRow) return -1;

      let newSeq: number;
      if (position === 'BEFORE') {
        const prevRow = this.db
          .prepare('SELECT MAX(seq) as maxSeq FROM list_store WHERE key = ? AND seq < ?')
          .get(key, pivotRow.seq) as { maxSeq: number | null };
        ...
        const nextRow = this.db
          .prepare('SELECT MIN(seq) as minSeq FROM list_store WHERE key = ? AND seq > ?')
          .get(key, pivotRow.seq) as { minSeq: number | null };
  ...
  async lmove(
    ...
    const tx = this.db.transaction(() => {
      ...
      const srcRow = this.db
        .prepare(
          `SELECT seq, value FROM list_store WHERE key = ? ORDER BY seq ${orderClause} LIMIT 1`
        )
        .get(source) as { seq: number; value: string } | undefined;
      if (!srcRow) return null;
      this.db.prepare('DELETE FROM list_store WHERE key = ? AND seq = ?').run(source, srcRow.seq);
      ...
      if (destDir === 'LEFT') {
        ...
        const maxSeqRow = this.db
          .prepare('SELECT MAX(seq) as maxSeq FROM list_store WHERE key = ?')
          .get(destination) as { maxSeq: number | null };
  ...
  async blpop(keys: string[], timeout: number): Promise<{ key: string; element: string } | null> {
    for (const key of keys) {
      ...
      const tx = this.db.transaction(() => {
        const row = this.db
          .prepare('SELECT seq, value FROM list_store WHERE key = ? ORDER BY seq ASC LIMIT 1')
          .get(key) as { seq: number; value: string } | undefined;
        if (!row) return null;
        this.db.prepare('DELETE FROM list_store WHERE key = ? AND seq = ?').run(key, row.seq);
        this._cleanupListIfEmpty(key);
  ...
  async lmpop(
    ...
    for (const key of keys) {
      ...
      const tx = this.db.transaction(() => {
        ...
        const rows = this.db
          .prepare(
            `SELECT seq, value FROM list_store WHERE key = ? ORDER BY seq ${orderClause} LIMIT ?`
          )
          .all(key, effectiveCount) as { seq: number; value: string }[];
