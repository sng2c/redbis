...
export const bitmapMethods = {
  ...
  _sqliteBytesToString(bytes: number[]): string {
  ...
  async setbit(key: string, offset: number, value: 0 | 1): Promise<number> {
    ...
    this.db
      ...
      .run(key, newValue, existingExpiresAt);
    return oldBit;
  },
  ...
  async bitop(
    ...
    if (operation === 'NOT') {
      ...
      this.db
        ...
        .run(destkey, resultStr);
      return result.length;
    }
    if (keys.length === 0) {
      this.evictExpired(destkey);
      this.db.prepare('DELETE FROM kv_store WHERE key = ?').run(destkey);
      return 0;
    }
    ...
    for (let i = 0; i < maxLen; i++) {
      if (operation === 'AND') {
        let val = 0xff;
        for (const arr of srcArrays) val &= i < arr.length ? arr[i] : 0;
        result[i] = val;
  ...
  async bitfieldRo(
    ...
    const opsWithOverflow: Array<{
      ...
      value?: number;
      overflow?: 'WRAP' | 'SAT' | 'FAIL';
    }> = operations.map((op) => ({ ...op, overflow: 'WRAP' as const }));
