...
export class SqliteStorage {
  ...
  migrate(): void {
    ...
    this.db
      ...
      .run();
    this.db
      .prepare('CREATE INDEX IF NOT EXISTS idx_zset_score ON zset_store(key, score, member)')
      .run();
    ...
    this.db
      ...
      .run();
    this.db.prepare('CREATE INDEX IF NOT EXISTS idx_geo_store_key ON geo_store(key)').run();
    ...
    this.db
      ...
      .run();
    this.db
      .prepare('CREATE INDEX IF NOT EXISTS idx_stream_entries_key_id ON stream_entries(key, id)')
      .run();
    this.db
      .prepare(
  ...
  evictExpired(key: string): void {
    const result = this.db
      .prepare('DELETE FROM kv_store WHERE key = ? AND expires_at IS NOT NULL AND expires_at <= ?')
      .run(key, Date.now());
    if (result.changes > 0) {
  ...
  async msetnx(pairs: Array<{ key: string; value: string }>): Promise<boolean> {
    ...
    const existing = this.db
      .prepare(`SELECT COUNT(*) as cnt FROM kv_store WHERE key IN (${placeholders})`)
  ...
  async setrange(key: string, offset: number, value: string): Promise<number> {
    this.evictExpired(key);
    const row = this.db
      .prepare('SELECT value, type, expires_at FROM kv_store WHERE key = ?')
      .get(key) as { value: string; type: string; expires_at: number | null } | undefined;
    ...
    this.db
      .prepare('INSERT OR REPLACE INTO kv_store (key, value, type, expires_at) VALUES (?, ?, ?, ?)')
  ...
  async setnx(key: string, value: string): Promise<boolean> {
    ...
    if (row) return false;
    this.db
      .prepare("INSERT INTO kv_store (key, value, type, expires_at) VALUES (?, ?, 'string', NULL)")
      .run(key, value);
    return true;
  }
  ...
  async rename(oldKey: string, newKey: string): Promise<void> {
    ...
    const row = this.db
      .prepare('SELECT value, type, expires_at FROM kv_store WHERE key = ?')
      .get(oldKey) as { value: string; type: string; expires_at: number | null } | undefined;
    if (!row) {
  ...
  async copy(source: string, destination: string): Promise<boolean> {
    ...
    const row = this.db
      .prepare('SELECT value, type, expires_at FROM kv_store WHERE key = ?')
      .get(source) as { value: string; type: string; expires_at: number | null } | undefined;
    if (!row) return false;
  ...
  async randomkey(): Promise<string | null> {
    this.evictAllExpired();
    const row = this.db.prepare('SELECT key FROM kv_store ORDER BY RANDOM() LIMIT 1').get() as
      | { key: string }
      | undefined;
    return row?.key ?? null;
  }
  ...
  async unlink(keys: string[]): Promise<number> {
    ...
    this.db.prepare(`DELETE FROM kv_store WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM hash_store WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM list_store WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM set_store WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM zset_store WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM geo_store WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM stream_entries WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM stream_meta WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM stream_groups WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM stream_consumers WHERE key IN (${placeholders})`).run(...keys);
    this.db.prepare(`DELETE FROM stream_pending WHERE key IN (${placeholders})`).run(...keys);
    return existing.cnt;
  }
  ...
  async scan(
    ...
    const rows = this.db
      .prepare('SELECT key, rowid FROM kv_store WHERE rowid > ? ORDER BY rowid')
    ...
    const remainingRows = this.db
      .prepare('SELECT COUNT(*) as cnt FROM kv_store WHERE rowid > ?')
