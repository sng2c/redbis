...
_xtrimInternal(key: string, strategy: 'MAXLEN' | 'MINID', threshold: string | number, approx: boolean, limit?: number): number {
    const cntRow = this.db.prepare('SELECT COUNT(*) as cnt FROM stream_entries WHERE key = ?').get(key) as { cnt: number };
    ...
    if (strategy === 'MAXLEN') {
      ...
      if (rows.length > 0) {
        ...
        if (metaRow) {
          ...
          const newMaxDeletedId = this._compareStreamId(metaRow.max_deleted_id, lastDeletedId) > 0 ? metaRow.max_deleted_id : lastDeletedId;
