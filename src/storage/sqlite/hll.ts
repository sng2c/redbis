...
export const hllMethods = {
  ...
  async pfadd(key: string, elements: string[]): Promise<number> {
    ...
    if (!row) {
      ...
      registers = this._hllDecode(row.value);
    ...
    if (changed || !row) {
      this.db
        .prepare(
          "INSERT OR REPLACE INTO kv_store (key, value, type, expires_at) VALUES (?, ?, 'hyperloglog', ?)"
        )
        .run(key, this._hllEncode(registers), existingExpiresAt);
    }
    return changed ? 1 : 0;
  },
  ...
  async pfcount(keys: string[]): Promise<number> {
    ...
    if (keys.length === 1) {
      const row = this.db.prepare('SELECT value FROM kv_store WHERE key = ?').get(keys[0]) as
        | { value: string }
        | undefined;
      if (!row) return 0;
  ...
  async pfmerge(destkey: string, sourceKeys: string[]): Promise<void> {
    ...
    this.db
      .prepare(
        "INSERT OR REPLACE INTO kv_store (key, value, type, expires_at) VALUES (?, ?, 'hyperloglog', NULL)"
      )
      .run(destkey, this._hllEncode(merged));
  },
};
