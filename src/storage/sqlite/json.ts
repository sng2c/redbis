...
export const jsonMethods = {
    ...
    while (i < p.length) {
      if (p[i] === '[') {
        ...
        if (/^\d+$/.test(content)) {
          ...
          segments.push({ type: 'field', name: content.replace(/^['"]|['"]$/g, '') });
        }
        i = end + 1;
        if (i < p.length && p[i] === '.') i++;
      } else {
  ...
  _sqliteJsonResolvePath(
    root: any,
    path: string
  ): { parent: any; key: string | number; value: any }[] {
    if (path === '$' || path === '.' || path === '')
  ...
  _sqliteDeepMerge(target: any, source: any): any {
    ...
    for (const key of Object.keys(source)) {
      if (source[key] === null) {
        ...
        result[key] = this._sqliteDeepMerge(result[key], source[key]);
      } else {
  ...
  async jsonSet(
    ...
    if (path === '$' || path === '') {
      ...
      this.db
        .prepare(
          "INSERT OR REPLACE INTO kv_store (key, value, type, expires_at) VALUES (?, ?, 'json', ?)"
        )
        .run(key, JSON.stringify(parsedValue), row?.expires_at ?? null);
      return 'OK';
    }
    ...
    if (!row) {
      ...
      this.db
        .prepare(
          "INSERT OR REPLACE INTO kv_store (key, value, type, expires_at) VALUES (?, ?, 'json', NULL)"
        )
        .run(key, JSON.stringify(parsedValue));
      return 'OK';
    }
    ...
    for (const r of resolved) {
      if (r.parent !== null) r.parent[r.key] = parsedValue;
    }
    this.db
      .prepare(
        "INSERT OR REPLACE INTO kv_store (key, value, type, expires_at) VALUES (?, ?, 'json', ?)"
      )
      .run(key, JSON.stringify(root), row.expires_at);
    return 'OK';
  },
  ...
  async jsonDel(key: string, path?: string): Promise<number> {
    this.evictExpired(key);
    const row = this.db.prepare('SELECT value, type FROM kv_store WHERE key = ?').get(key) as
  ...
  async jsonArrappend(key: string, path: string, values: string[]): Promise<(number | null)[]> {
    ...
    for (const r of resolved) {
      if (Array.isArray(r.value)) {
        ...
        if (r.parent !== null) r.parent[r.key] = r.value;
